...
import com.matthewperiut.chisel.legacy.data.Recipes.RecipeKey;
...
public class BlockPack {
    ...
    public BlockPack(String packDir) {
        ...
        if (!blockPackFolder.exists()) {
            ...
            if (blockPackZip.exists()) {
                ...
                com.matthewperiut.chisel.Chisel.LOGGER.warn(msg);
            } else {
    ...
    public void registerTextures(RuntimeResourcePack resourcePack) {
        ...
        if (!textureFolder.exists()) {
            ...
            com.matthewperiut.chisel.Chisel.LOGGER.info(msg);
            return;
        }
        ...
        for (int i = 0; i < textureFiles.size(); i++) {
            ...
            resourcePack.addAsset(new Identifier(com.matthewperiut.chisel.Chisel.MOD_ID, "textures/blocks/" + packDir + region), readFile(textureFile));
        }
    }
    ...
    public void registerLang(RuntimeResourcePack resourcePack) {
        ...
        for (int i = 0; i < langFiles.size(); i++) {
            ...
            resourcePack.addAsset(new Identifier(com.matthewperiut.chisel.Chisel.MOD_ID, "lang/" + region), readFile(langFile));
        }
    }
