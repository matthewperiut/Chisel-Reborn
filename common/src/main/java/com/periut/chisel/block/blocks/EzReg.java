...
public class EzReg
{
    public static void Reg(String group, String name)
    {
        ...
        Block baseBlock = glass ? Blocks.GLASS : (nether_brick ? Blocks.NETHER_BRICKS : (purpur ? Blocks.PURPUR_BLOCK : (quartz ? Blocks.QUARTZ_BLOCK : BuiltInRegistries.BLOCK.getValue(baseBlockIdentifier))));
        ...
        if (redstone && pillar)
            ...
            blockSupplier = RegistryHelper.register(BuiltInRegistries.BLOCK, block_id, () -> new TransparentBlock(BlockBehaviour.Properties.ofFullCopy(baseBlock).setId(block_key)));
        else
